from PyQt5.QtWidgets import QWidget, QLabel

class latian2(QWidget):
    def __init__(self):
        super().__init__()
        self.setupUi()

    def setupUi(self):
        self.resize(400, 200)
        self.move(300, 300)
        self.setWindowTitle('Latihan 2 <Tag HTML>')
        self.label1 = QLabel('<h1>Rekayasa <font color=red>Perangkat <font color=blue>Lunak </font></h1>')
        self.label1.move(10, 10)
        self.label1.setParent(self)
        self.label2 = QLabel('<h2><b>Peserta Praktikum Pemrograman GUI</b></h2>')
        self.label2.move(10,50)
        self.label2.setParent(self)
        self.label3 = QLabel('<ol><li>Monkey D Luffy</li><li>Roronoa Zorro</i><li>Takiya Genji</li></ol>')
        self.label3.move(20,80)
        self.label3.setParent(self)
